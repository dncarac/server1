...
def server_post():
    ...
    '''
    _LOG.trace("Enter Server.sbackup_server_folder)
#--------------------------------------------------------------------------------------------------
    # Log data values
    _LOG.debug("  Start sbackup_server_folder--")
    ...
    print("session data: %s" % session.data['tdata'])
#--------------------------------------------------------------------------------------------------
    _LOG.trace("Leave Server.sbackup_server_folder)
...
if __name__ == '__main__':
    ...
    def start_server():
        _LOG.trace("Start sbackup_server_folder)
        serve(bt_app)
...
#     assert r.headers['Ack'] == 'True'
    ...
    data = {cfg.MSG_TYPE_KEY: "SendFile"}
    with open(cfg.TASK_PATH + 'breast task 5.tsk', 'rb') as f:
        r = rq.post('http://Dell:8080', files={'upfile': f})
